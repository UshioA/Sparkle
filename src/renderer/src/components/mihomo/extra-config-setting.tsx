  ...
  return (
    <SettingCard header="附加配置">
      ...
      <div className="px-4 pb-3 text-xs text-gray-500 leading-5">
        ...
        <code>+proxies</code> / <code>+proxy-groups</code> / <code>+rules</code> 追加到对应列表前面（<code>key+</code> 追加到后面），
        其它键按对象合并（<code>proxy-providers</code> 这类对象键直接写即可）。
        <code>target-core</code> 和 <code>requires-version</code> 用来限定只有合适的内核才会注入；
        <code>group-inject</code> 可以把新加的组塞进「节点选择」这类已有选择组里（不用再写覆写脚本）；
        文件不存在或解析失败时自动跳过，不影响内核启动。
      </div>
    </SettingCard>
  )
}
